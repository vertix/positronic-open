...
# TODO: Make "call-only" output ports that are generated by a function, i.e.
# systems don't write to them but define a function that produces them.

# TODO: Intoduce notion of "closed" ports, so that the output port can be closed
# and the binded input port knows about it. In this case, only the producer
# systems must respect the should_stop flag, and all other just exit when process
# all the data.
