...
async def test_simple_composition():
    ...
    # Compose all systems
    system = compose(source, processor.bind(counter=source.outs.counter, in_data=source.outs.data),
                     sink.bind(data=processor.outs.processed, counter_accumulated=processor.outs.counter_accumulated))
...
async def test_nested_composition():
    ...
    composed_lvl1 = compose(source,
                            processor.bind(counter=source.outs.counter, in_data=source.outs.data),
                            outputs={
                                'result': processor.outs.processed,
                                'counter': processor.outs.counter_accumulated
                            })

    composed_lvl2 = compose(composed_lvl1,
                            sink.bind(data=composed_lvl1.outs.result, counter_accumulated=composed_lvl1.outs.counter),
                            outputs={
...
async def test_composition_with_not_included_components_produces_exception():
    ...
    with pytest.raises(CompositionError):
        compose(processor.bind(counter=not_included_source.outs.counter, in_data=not_included_source.outs.data),
                sink.bind(data=processor.outs.processed, counter_accumulated=processor.outs.counter_accumulated))
...
async def test_composition_with_not_included_input_component_produces_exception():
    ...
    with pytest.raises(CompositionError, match="Input mappings reference components not in composition"):
        compose(processor_1, inputs={'data': (processor_2, 'in_data'), 'counter': (processor_2, 'counter')})
...
async def test_composition_with_not_included_output_component_produces_exception():
    ...
    processor_2.bind(in_data=processor_1.outs.processed, counter=processor_1.outs.counter_accumulated)

    with pytest.raises(CompositionError, match="Output mappings reference components not in composition"):
        compose(source,
                processor_1.bind(counter=source.outs.counter, in_data=source.outs.data),
                outputs={
...
async def test_exposed_outputs():
    ...
    system = compose(source,
                     processor.bind(counter=source.counter, in_data=source.outs.data),
                     outputs={
                         'result': processor.outs.processed,
                         'counter': processor.outs.counter_accumulated
                     })
    ...
    sink = DataSink().bind(data=system.outs.result, counter_accumulated=system.outs.counter)
...
async def test_invalid_input():
    ...
    processor = Processor()
    system = compose(processor, inputs={'valid_input': (processor, 'in_data')})
...
async def test_none_not_allowed_as_output():
    ...
    with pytest.raises(
            CompositionError,
            match="Output 'result' must be either an OutputPort or an async function, got <class 'NoneType'>"):
        compose(source, processor, outputs={'result': None})
