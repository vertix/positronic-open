...
class TestSignalWindow:
    def test_window_empty_Signal(self, tmp_path):
        Signal = create_Signal(tmp_path, [], "empty.parquet")
        values, timestamps = Signal.window(1000, 2000)
        assert len(values) == 0 and len(timestamps) == 0

    def test_window_no_data_in_range(self, tmp_path):
        Signal = create_Signal(tmp_path, [(42, 1000), (43, 2000)])
        values, timestamps = Signal.window(3000, 4000)
        assert len(values) == 0 and len(timestamps) == 0

    def test_window_inclusive_bounds_scalar(self, tmp_path):
        Signal = create_Signal(tmp_path, [(42, 1000), (43, 2000), (44, 3000)])
        values, timestamps = Signal.window(1000, 3000)
    ...
    def test_window_partial_range_scalar(self, tmp_path):
        ...
        values, timestamps = Signal.window(1500, 3500)
    ...
    def test_window_vector_data(self, tmp_path):
        ...
        values, timestamps = Signal.window(1000, 2500)
        assert len(values) == 2
    ...
    def test_window_single_point(self, tmp_path):
        Signal = create_Signal(tmp_path, [(42, 1000), (43, 2000), (44, 3000)])
        values, timestamps = Signal.window(2000, 2000)
...
class TestSignalWriterAppend:
    def test_append_increasing_timestamps(self, tmp_path):
        Signal = create_Signal(tmp_path, [(42, 1000), (43, 2000), (44, 3000)])
        values, timestamps = Signal.window(0, 10000)
        np.testing.assert_array_equal(values, [42, 43, 44])
        np.testing.assert_array_equal(timestamps, [1000, 2000, 3000])
