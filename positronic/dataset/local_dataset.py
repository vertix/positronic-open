...
def _is_six_digit_dir(p: Path) -> bool:
    """Return True if path is a directory named as zero-padded 6-digit number."""
    name = p.name
    return p.is_dir() and len(name) == 6 and name.isdigit()


def _ensure_block_dir(root: Path, episode_id: int) -> Path:
    block_start = (episode_id // 1000) * 1000
    block_dir = root / f"{block_start:06d}"
...
class LocalDataset(Dataset):
    ...
    Layout:
      root/
        000000/            # block for episodes [0..999]
          000000/          # episode 0 (full 6-digit id)
          000001/          # episode 1
          ...
        001000/            # block for episodes [1000..1999]
          001000/          # episode 1000
    ...
    def _scan(self) -> None:
        ...
        if not self.root.exists():
            return
        for block_dir in sorted([p for p in self.root.iterdir() if _is_six_digit_dir(p)], key=lambda p: p.name):
            for ep_dir in sorted([p for p in block_dir.iterdir() if _is_six_digit_dir(p)], key=lambda p: p.name):
...
class LocalDatasetWriter(DatasetWriter):
    ...
    - Stores episodes under root / {block:06d} / {episode_id:06d}
    ...
    def _compute_next_episode_id(self) -> int:
        ...
        for block_dir in self.root.iterdir():
            if not _is_six_digit_dir(block_dir):
                continue
            for ep_dir in block_dir.iterdir():
                if not _is_six_digit_dir(ep_dir):
                    continue
                eid = int(ep_dir.name)
                if eid > max_id:
                    max_id = eid
